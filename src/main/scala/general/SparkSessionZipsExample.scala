...
object SparkSessionZipsExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      "SELECT COUNT(zip), SUM(pop), city FROM zips_hive_table WHERE state = 'CA' GROUP BY city ORDER BY SUM(pop) DESC")
      .show(10, false)
    ...
    val resultsCityLens = spark.sql(
      "SELECT city, cityLength(city) as city_length FROM hive_zips_table ORDER BY city_length DESC")
